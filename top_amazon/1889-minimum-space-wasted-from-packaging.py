"""
You have n packages that you are trying to place in boxes, one package in each box. There are m suppliers 
that each produce boxes of different sizes (with infinite supply). A package can be placed in a box if the 
...
can fit the packages of size-2 and size-3 into two boxes of size-4 and the package with size-5 into a box of size-8. 
...
the packages inside boxes. Since the answer may be large, return it modulo 109 + 7.
...
class Solution:
    def minWastedSpace(self, packages: List[int], boxes: List[List[int]]) -> int:
        ...
        for box_set in boxes:
            box_set.sort()
            if box_set[-1] < packages[-1]: continue
                
            acc, i = 0, 0 
            for box_size in box_set:
                j = bisect.bisect_right(packages, box_size, i)
                acc += box_size * (j - i)
                i = j
                
            space = min(acc, space)
            
        return -1 if space == float('inf') else (space - sum(packages)) % (10 ** 9 + 7)
