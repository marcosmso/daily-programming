...
class Solution:
    def maxArea(self, h: int, w: int, horizontalCuts: List[int], verticalCuts: List[int]) -> int:
        ...
        def getMaxDiff(cuts, side_len):
            max_diff = float("-inf")
            prev = 0
            for cut in cuts:
                max_diff = max(max_diff, cut - prev)
                prev = cut
            max_diff = max(max_diff, side_len - prev)
            return max_diff
            
        max_horizontal_diff = getMaxDiff(verticalCuts, w)
        max_vertical_diff = getMaxDiff(horizontalCuts, h)
