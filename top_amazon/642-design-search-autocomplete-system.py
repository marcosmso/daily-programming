...
typed sentence and times[i] is the corresponding number of times the sentence was typed. For each input character except 
...
class AutocompleteSystem:
    ...
    def __init__(self, sentences: List[str], times: List[int]):
        ...
        for sentence, freq in zip(sentences, times):
            self.addToTrie(self.trie, sentence, freq)
    
    def addToTrie(self, trie, sentence, freq):
        index = 0
        while index < len(sentence):
            trie = trie.setdefault(sentence[index], {}) 
            index += 1
    ...
    def findSugestions(self, trie, preffix):    
        self.sugestions = []
        index = 0
        while index < len(preffix):
            if preffix[index] in trie:
                trie = trie[preffix[index]]
                index += 1
            else:
                return
        
        def helper(trie, currentWord):
            if "#" in trie:
                self.sugestions.append(Wrapper(currentWord, trie["#"]))
            for letter in trie:
                if letter == "#":
                    continue
                helper(trie[letter], currentWord + letter)
        
        helper(trie, preffix)
    
    def input(self, c: str) -> List[str]:
        if c == '#':
            self.addToTrie(self.trie, self.typed, 1) 
            self.typed = ""
            return
        
        self.typed = self.typed + c
        self.findSugestions(self.trie, self.typed)
        self.sugestions.sort()

        if len(self.sugestions) <= 3:
            return [w.sentence for w in self.sugestions]
        return [w.sentence for w in self.sugestions[0:3]]