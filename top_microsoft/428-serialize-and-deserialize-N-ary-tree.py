...
Or you can follow LeetCode's level order traversal serialization format, where each group of children is separated by the
null value.
...
class Codec:
    ...
    def serializeDFS(self, node):
        if not node:
            return
        numChildren = len(node.children) if node.children else 0
        code = str(node.val) + "#" + str(numChildren)
        self.data.append(code)
        if node.children:
            for child in node.children:
                self.serializeDFS(child)
    ...
    def deserializeDFS(self):
        ...
        code = self.data.popleft()
        val, numChildren = code.split("#")
        val, numChildren = int(val), int(numChildren)
        ...
        i, children = 0, []
        while i < numChildren:
            children.append(self.deserializeDFS()) 
            i+=1
