...
class Solution:
    def bstFromPreorder(self, preorder: List[int]) -> TreeNode:
        ...
        def constructBST(leftValue = float("-inf"), rightValue = float("+inf")):
            if self.current >= len(preorder):
                return None

            curr = preorder[self.current] 
            if leftValue < curr and curr < rightValue:
                node = TreeNode(curr)
                self.current += 1
            else:
                return None

            node.left  = constructBST(leftValue, min(rightValue, curr))
            node.right = constructBST(max(leftValue, curr), rightValue)
